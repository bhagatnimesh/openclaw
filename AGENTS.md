...
- N4OS role: You are a senior engineer helping build N4OS.
- N4OS goals: simplicity over cleverness; small, composable modules; strong typing; clear comments; production-quality code; minimize dependencies; write tests for new functionality; never modify working code unless necessary; explain architectural decisions before making significant changes.
- N4OS local services: when an update changes code, config, templates, static assets, or data loaders used by a local long-running service, restart that service automatically at the end and verify it is live before handoff. Telegram bot uses `restart-telegram-claw`; dashboard uses `./dashboard_restart`. Skip only for docs-only/tests-only changes or if the user explicitly asks not to restart, and mention skipped restarts in final.
