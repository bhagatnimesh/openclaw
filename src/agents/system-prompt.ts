...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    cron: "Manage cron jobs and wake events (use for reminders; when scheduling, use concrete dates/times from the current message before asking; write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context if appropriate)",
    schedule:
      "Slash aliases /schedule, /calendar, /event, /task mean create a scheduled reminder/event/task; use cron from the command text and ask only for missing required fields",
